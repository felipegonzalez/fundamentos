...
En particular, podemos estudiar la **dispersión y
...
de la escala de medición de los datos tal que aproximadamente una fracción $f$ de los datos son menor o igual a $q(f)$.
```

Hay varias maneras de definir esto con precisión (ver [@cleveland93]): 
...
cada $x_{(i)}$ definimos  
$$f_i = i / N$$
entonces definimos el cuantil $q(f_i)=x_{(i)}$. Para cualquier calcular
$q(f)$ donde $f$ está entre $f_i$ y $f_{i+1}$ interpolamos linealmente 
...
**Observación**: en la práctica, es más conveniente usar $f_i= \frac{i - 0.5}{N}$,
pues más adelante queremos comparar con 
distribuciones teóricas no tienen definido el cuantil 0 y el 1, pues tienen
...
$$\hat{F}(x) = x_{(i)}$$
si $x_{(i)} \leq x_{(i+1)}$. Nótese que $\hat{F}(q(f_i)) = i/N = f_i$ (demuéstralo).
...
**Ajustando familias paramétricas** El modelo de *regresion lineal* ajusta una recta
...
\end{align}
donde $x_k \in \{x_1, \ldots, x_N\}$.
```


```{block, type = 'mathblock'}
**Observación:** Notemos que el ajuste tradicional por medio de mínimos cuadrados considera un ajuste *global*
con pesos uniformes
\begin{align}
w_n(x) = \frac1N.
\end{align}
...
```{block, type = 'mathblock'}
**Interpretación:** En la práctica, los pesos $w_n(\cdot)$ contienen a su vez un conjunto de parámetros
que dictan el comportamiento del ajuste local. Si por ejemplo tomamos en cuenta
pesos Gaussianos
\begin{align}
w_n(x) = \text{exp}\left( -\frac{ (x - x_n)^2 }{2\sigma^2} \right),
\end{align}
el parámetro $\sigma$ dicta el efecto local de los puntos ancla $x_n$. Dicho de otro
modo, dicta la vecindad de incidencia de los datos $x_n.$
```
...
\end{align}
donde $p$ es el máximo grado del polinomio. De esta forma, $p$ controla el suavizamiento
de la curva. El caso de recta lineal claramente es para $p=1.$
```
...
```{r, echo = FALSE, fig.height=2, echo = FALSE,  out.width = '90%', fig.align= 'center', cache=TRUE, fig.asp = 0.35}
